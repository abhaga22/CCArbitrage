...
class Fixerio:
    ...
    def _in_cache(self, base, symbols, date=_date()):
        ...
        try:
            if symbols is not None:
                symbols = [x.strip() for x in symbols.strip().split(',')]
    ...
    def get_rates(self, date=LATEST, base=DEFAULT_BASE, symbols=None):
        ...
        if self._in_cache(base, symbols, _date(date)):
            return self._return_cache(base, symbols)
        else:
            url = BASE_URL + date
            if symbols is None:
                payload = dict(base=base)
